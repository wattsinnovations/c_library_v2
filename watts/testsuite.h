...
static void mavlink_test_eeprom_page(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_eeprom_page_t packet_in = {
        963497464,963497672,"IJKLMNOPQRSTUVW"
    };
    ...
    mavlink_msg_eeprom_page_decode(&msg, &packet2);
        ...
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_eeprom_page_pack(system_id, component_id, &msg , packet1.index , packet1.page_data , packet1.page_num );
    mavlink_msg_eeprom_page_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_eeprom_page_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.index , packet1.page_data , packet1.page_num );
    mavlink_msg_eeprom_page_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_eeprom_page_decode(last_msg, &packet2);
        ...
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_eeprom_page_send(MAVLINK_COMM_1 , packet1.index , packet1.page_data , packet1.page_num );
    mavlink_msg_eeprom_page_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
